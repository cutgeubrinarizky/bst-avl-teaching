...
function insertAVL(node, value, logs = []) {
  if (!node) return makeNode(value);
...
function buildTree(values, mode) {
  let root = null;
  const logs = [];
...
function getLayout(root, options = {}) {
...
function insertLeafOnly(root, value) {
...
function depthOf(root, targetVal, d = 0) {
  if (!root) return -1;
...
function findDeepestUnbalanced(root) {
  let best = null;
  function walk(n) {
    if (!n) return;
    walk(n.left);
...
function rotateNodeInTree(root, pivotVal, rotateFn) {
  if (!root) return null;
...
const DELETION_OPERATION_STEPS = [
  {
    short: "Awal",
...
function buildAVLRotationLab(caseName) {
...
function TreeCanvas({ root, selected, onSelect, mode, highlights = {}, layoutOptions = {} }) {
